...
class OvercookedGameServer:
    def __init__(self, host="127.0.0.1", port=50007, horizon=100, layout="sr_testing"):
        self.host = host
        self.port = port
        self.horizon = horizon
        self.layout = layout
        self.env = OvercookedAIEnv(player_id=0, horizon=self.horizon, layout=self.layout)
    ...
    def send_state(self, conn):
        game_state = {
            "base_env_state": self.env.base_env.state, 
            "terrain_mtx": self.env.base_env.mdp.terrain_mtx
        }
    ...
    def run(self):
        ...
        while True:
            ...
            if command:
                ...
                self.env.execute_action(action_name, args)

            time.sleep(0.1)

class OvercookedAIEnv():

    def __init__(self, player_id=0, horizon=100, layout="asymmetric_advantages_tomato", render=False):
        """
...
class OvercookedRouteProblem(Problem):
    ...
    def goal_test(self, state_node, goal_node=None):
        ...
        counter_objects = Problem.extra.mdp.get_counter_objects_dict(Problem.extra.state)
